...
def train_model(args):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (inputs, targets) in enumerate(dataloader):
            ...
            if epoch % 5 == 0:
                np.savez(f'dataMP/level{step_dataloader + 1}_loss{criterium_idx}_epoch{epoch}_batch{i}.npz',
...
#                 firing_rate = torch.count_nonzero(spikes) / 30000
#                 print("torch working")
...
#             if epoch % 5 == 0:
#                 np.savez(f'dataMP/level{step_dataloader + 1}_loss{criterium_idx}_epoch{epoch}_model{model_idx}.npz',
#                          loss=loss.item(),
#                          spikes=spikes.detach().numpy(),
#                          input_weights=model.l1.weight.data.detach().numpy(),
#                          rec_weights=model.rlif1.recurrent.weight.data.detach().numpy(),
